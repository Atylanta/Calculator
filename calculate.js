...
function clickPercent(){
    //Описываем условия написания знака
    if (arrayMemoryNumbers.length < 1 && !changes) { //если экран пуст
        display.value = 0;
    } else if (changes) { // Если мы ввели новое число
        if (changes && arrayMemoryNumbers.length < 1) { //если на экране только 1 число
            display.value = 0;
        } else { //если на экране больше 1 числа
            display.value = arrayMemoryNumbers[arrayMemoryNumbers.length-1] / 100 * display.value;
        }
    } else if (!changes && display.value == resultEquall) { //Если на экране - результат функции равно
        return display.value;
    } else { //Если предыдущее значение - знак или это посчитанный результат
        return display.value;
    }
}
...
function clickRoot(){
    //Описываем условия написания знака
    if (display.value === 0) { //Если экран полностью пуст
        display.value = 0;
    } else if (changes) { // Если мы ввели новое число
        display.value = Math.sqrt(display.value);
    } else if (!changes && display.value == resultEquall) { //Если мы хотим получить кв.крень результата
        display.value = Math.sqrt(display.value);
    } else { //Если предыдущее значение - знак или это посчитанный результат
        return display.value;
    }
...
function clickPower(){
    ...
    } else if (!changes && display.value == resultEquall) { //Если мы хотим получить результат в квадрате
        display.value = Math.pow(display.value, 2);
    } else { //Если предыдущее значение - знак или это посчитанный результат
        return display.value;
    }
...
function clickNumber(value){
    return function() {
        if (display.value === '0') { //Если экран пуст
            display.value = value;
            changes = true;
        } else {
            if (changes) { //Если мы вводим число
                display.value += value;
                changes = true;
            } else { //Если предыдущее значение - математический знак
                display.value = value;
                changes = true;
            }
        }
...
function clickSimpleOperation(value){
    return function () {
        if (display.value === 0) { //Если экран полностью пуст
            display.value = 0;
        } else {
            if (changes) { // Если мы ввели новое число
                arrayMemoryNumbers.push(display.value);
                displayMemory.value += display.value + value;
                arrayMemoryOperations.push(value);
                changes = false;
            } else if (!changes && display.value == resultEquall) { // Если мы посчитали результат и хотим произвести с ним некоторые операции
                arrayMemoryNumbers.push(display.value);
                displayMemory.value = display.value + value;
                arrayMemoryOperations.push(value);
                changes = false;
                ...
                if (arrayMemoryOperations[num] === value && !changes) { //Если мы нажимаем знак повторно
                    return display.value;
                } else { // Если мы не вводили цифры и хотим сменить знак
...
function clickPlusMinus(){ //Меняем плюс на минус и наоборот перед выражением
    if (display.value[0] == '-') {
...
function clickDot(){ //Десятичная точка
...
function clickEquall(){
    //Добавляем в массив последнее введенное значение
    arrayMemoryNumbers.push(display.value);
    displayMemory.value += display.value;
    changes = false;

    //Выполняем действия умножения или деления
    ...
    //Выполняем действия сложения и вычитания
    for (let j = 0; j <= arrayMemoryOperations.length; j++) {
        for (let i = 0; i <= arrayMemoryOperations.length; i++) {
            if (arrayMemoryOperations[i] === '+' ||
                arrayMemoryOperations[i] === '-') {
